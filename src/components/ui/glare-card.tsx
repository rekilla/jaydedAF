...
export const GlareCard = ({
  ...
  const containerStyle = {
    ...
    "--foil-size": "100%",
    "--opacity": "0",
    "--radius": "12px", // Adjusted radius for a slightly less rounded look
    "--easing": "ease",
    "--transition": "var(--duration) var(--easing)",
  } as React.CSSProperties; // Use React.CSSProperties type
  ...
  const updateStyles = () => {
    if (refElement.current) {
      // console.log(state.current); // Removed console log
  ...
  return (
    <div
      ...
      className="relative isolate [contain:layout_style] [perspective:600px] transition-transform duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] will-change-transform w-full max-w-[320px] [aspect-ratio:17/21]" // Made width responsive, kept aspect ratio
      ref={refElement}
      onPointerMove={(event) => {
    ...
    >
      <div className="h-full grid will-change-transform origin-center transition-transform duration-[var(--duration)] ease-[var(--easing)] delay-[var(--delay)] [transform:rotateY(var(--r-x))_rotateX(var(--r-y))] rounded-[var(--radius)] border border-slate-800 hover:[--opacity:0.6] hover:[--duration:200ms] hover:[--easing:linear] hover:filter-none overflow-hidden">
        <div className="w-full h-full grid [grid-area:1/1] mix-blend-soft-light [clip-path:inset(0_0_0_0_round_var(--radius))] relative"> {/* Added relative positioning */}
